...
import { Plus, Search, Minus, Edit, Trash2, X, Package } from "lucide-react";
    ...
    <div className="flex flex-col gap-5 h-full">
      ...
      <div className="grid grid-cols-4 gap-4">
        {[
          ...
          { label: "Inventory Value", value: `₹${totalValue.toLocaleString("en-IN", { minimumFractionDigits: 2 })}`, color: "text-primary font-bold" },
        ].map((stat) => (
                ...
                return (
                  ...
                  >
                    <td className="py-3 px-3 font-semibold text-slate-800">
                      {item.name}
                      {isOut && (
                        <span className="ml-2 text-xs bg-red-100 text-red-600 px-1.5 py-0.5 rounded-full font-medium">Out</span>
                      )}
                      {isLow && !isOut && (
                        <span className="ml-2 text-xs bg-amber-100 text-amber-700 px-1.5 py-0.5 rounded-full font-medium">Low</span>
                      )}
                    </td>
                    ...
                    <td className="py-3 px-3 font-semibold text-slate-800">₹{item.sellingPrice}</td>
                    <td className="py-3 px-3 text-slate-500">₹{item.costPrice}</td>
                    ...
                    <td className="py-3 px-3 text-right">
                      <button
                        ...
                        className="p-1.5 rounded-lg text-slate-400 hover:text-primary hover:bg-primary/10 transition-colors mr-1"
      ...
      <Dialog open={modalMode !== null} onOpenChange={(open) => !open && setModalMode(null)}>
        <DialogContent className="sm:max-w-lg bg-white">
          ...
          <div className="grid grid-cols-2 gap-4 py-2">
            ...
            <div className="space-y-1.5">
              <Label className="text-slate-700">Cost Price (₹) *</Label>
              <Input
                type="number"
                min="0"
            ...
            <div className="space-y-1.5">
              <Label className="text-slate-700">Initial Stock *</Label>
              <Input
                type="number"
                min="0"
                value={form.stock}
            ...
            <div className="space-y-1.5">
              <Label className="text-slate-700">Low Stock Alert (&lt;)</Label>
              <Input
                type="number"
                min="0"
          ...
          <DialogFooter>
            ...
            <Button onClick={handleSave} className="bg-primary hover:bg-primary/90 text-white" data-testid="btn-save-product">
      ...
      <Dialog open={deleteTarget !== null} onOpenChange={(open) => !open && setDeleteTarget(null)}>
        <DialogContent className="sm:max-w-sm bg-white">
          <DialogHeader>
            <DialogTitle className="text-slate-800">Delete Product?</DialogTitle>
          </DialogHeader>
          <p className="text-slate-600 text-sm py-2">
            Are you sure you want to remove <strong>{deleteTarget?.name}</strong> from inventory?
