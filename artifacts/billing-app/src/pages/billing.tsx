...
import { Plus, Printer, Share2, X, Percent, IndianRupee } from "lucide-react";
...
export default function BillingPage() {
  ...
  const handleAddToCart = () => {
    ...
    // Resolve discount: pct → direct %, flat → convert to effective %
    let discountPct = 0;
    if (discountMode === "pct") {
    ...
    if (existingIndex >= 0) {
      ...
      setCart((prev) => [
        ...prev,
        { productId: item.id, name: item.name, hsnCode: item.hsnCode, qty, rate, discount: discountPct, amount },
      ]);
    }
  ...
  // Only apply GST if globally enabled AND per-bill toggle is on
  ...
  const handleFinalize = () => {
    ...
    // Auto-deduct sold quantities from inventory
    ...
    <div className="flex flex-col md:flex-row gap-5 h-full overflow-hidden">
      ...
      <div className="w-full md:w-[360px] md:flex-shrink-0 glass-panel p-5 flex flex-col gap-4 no-print overflow-y-auto">
        ...
        <div className="space-y-3 border border-slate-200/60 rounded-xl p-4 bg-slate-50/50">
              ...
              <SelectContent>
                ...
                {inventory.map((item) => (
                  <SelectItem key={item.id} value={item.id}>
                    ...
                    <span className="text-slate-500 ml-2">{formatCurrency(item.sellingPrice, settings.currency)}</span>
                    {item.stock === 0 && <span className="ml-2 text-red-500 text-xs">Out</span>}
                    {item.stock > 0 && item.stock <= item.lowStockThreshold && (
                      <span className="ml-2 text-amber-500 text-xs">Low ({item.stock})</span>
                    )}
                  </SelectItem>
                ))}
              </SelectContent>
          ...
          {/* Discount — full width, stacked below Qty */}
          <div className="space-y-1.5">
        ...
        {/* GST toggle — only shown when globally enabled */}
        ...
        <Button size="lg" className="w-full bg-primary hover:bg-primary/90 text-white font-bold text-base shadow-md mt-auto" onClick={handleFinalize} disabled={cart.length === 0}>
              ...
              <table className="text-sm ml-auto">
                <tbody>
                  ...
                  {settings.currency === 'USD' && (
                    <tr>
                      <td className="text-slate-500 pr-3 py-0.5">Currency</td>
                      <td className="font-semibold text-slate-700">USD ($)</td>
                    </tr>
                  )}
                </tbody>
              </table>
            ...
            <div className="w-64 space-y-0">
              ...
              {totalDiscountGiven > 0 && (
                <div className="flex justify-between text-sm text-green-600 py-1.5 border-b border-slate-100">
                  <span>Discount</span>
                  <span>−{formatCurrency(totalDiscountGiven, settings.currency)}</span>
                </div>
              )}
              {gstApplicable && (
                <>
